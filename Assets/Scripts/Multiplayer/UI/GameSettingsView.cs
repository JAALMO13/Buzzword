...
public class GameSettingsView : View
{
    ...
    public override void Update()
    {
        ...
        if (selectedMin >= 0 && selectedMax >= 0 && (timeSlider.value > 0)){
            playButton.interactable = true;
            playButton.GetComponent<Image>().color = ColourMode.Instance.colour;
            playButton.GetComponentInChildren<TextMeshProUGUI>().color = Color.white;
        } // or slider val is not zero
        else {
            playButton.interactable = false;
            playButton.GetComponent<Image>().color = ColourMode.Instance.darkLightGrey;
            playButton.GetComponentInChildren<TextMeshProUGUI>().color = ColourMode.Instance.black;
        }
                ...
                default:
                    {
                        ...
                        if(-1 != selectedTime) oldTime = selectedTime;
                        selectedTime = -1;
                        break;
                    }
            }
        ...
        if (selectedMin >= 0)
        {
            ...
            if (oldMin > -1)
            {
                minLetters.transform.GetChild(oldMin).GetComponent<Image>().color = ColourMode.Instance.lightGrey;;
        ...
        if (selectedMax >= 0)
        {
            ...
            if (oldMax > -1)
            {
                maxLetters.transform.GetChild(oldMax).GetComponent<Image>().color = ColourMode.Instance.lightGrey;
        ...
        if (oldTime > -1)
        {
            timeLimit.transform.GetChild(oldTime).GetComponent<Image>().color = ColourMode.Instance.lightGrey;
    ...
    void CreateRoom()
    {
        ...
        for (int i = 0; i < roomManager.transform.childCount; i++)
        {
            ...
            if (r.max == max && r.min == min && r.time == time && !r.used)
            {
                // join this room
                Player.Instance.roomID = r.roomName;
                LobbyNetwork.JoinRoom(r.roomName, string.Empty);
                isRoom = true;
        ...
        if (!isRoom)
        {
            ...
            LobbyNetwork.CreateRoom(roomID, string.Empty, min, max, time);
            StartCoroutine(ExitAnimation<MatchView>());
        }
    }
